...
public class SkateFlagEncoder extends AbstractFlagEncoder {
    ...
    static final int MEAN_SPEED = 10;
    static final int FERRY_SPEED = 30;
    ...
    public SkateFlagEncoder(int speedBits, double speedFactor) {
        super(speedBits, speedFactor, 1);
        restrictions.addAll(Arrays.asList("skate", "access"));
        ...
        restrictedValues.add("track"); //Waldwege
    ...
    public EncodingManager.Access getAccess(ReaderWay way) {
        ...
        //Avoid the Elements which are Bad for the Skate Rols
        if (way.hasTag("inline_skates", "no")) // Skates Verboten
            return EncodingManager.Access.CAN_SKIP;
        if (way.hasTag("surface", "unpaved"))  //Ungepflastert
            return EncodingManager.Access.CAN_SKIP;
        if (way.hasTag("surface", "gravel"))  //Geröll
            return EncodingManager.Access.CAN_SKIP;
        if (way.hasTag("surface", "ground"))  //Boden
            return EncodingManager.Access.CAN_SKIP;
        if (way.hasTag("surface", "dirt"))    //Dreck
            return EncodingManager.Access.CAN_SKIP;
        if (way.hasTag("surface", "grass"))   //Gras
            return EncodingManager.Access.CAN_SKIP;
        if (way.hasTag("surface", "sand"))    //Sand
            return EncodingManager.Access.CAN_SKIP;
        if (way.hasTag("surface", "earth"))   //Erde
            return EncodingManager.Access.CAN_SKIP;
        if (way.hasTag("surface", "cobblestone"))
            return EncodingManager.Access.CAN_SKIP;
        if (way.hasTag("surface", "grass_paver"))
            return EncodingManager.Access.CAN_SKIP;
        if (way.hasTag("surface", "gravel"))
            return EncodingManager.Access.CAN_SKIP;
    ...
    void collect(ReaderWay way, TreeMap<Double, Integer> weightToPrioMap) {
        ...
        //if (way.hasTag("surface", "gravel"))
          //  weightToPrioMap.put(0d, AVOID_IF_POSSIBLE.getValue());
    }
